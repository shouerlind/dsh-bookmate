...
// skill registry so every agent preset's scope chain carries it, plus the two
// model tools the skill drives: `book_search` (OpenLibrary HTTP adapter) and
// `book_rank` (the recommend() engine behind a tool — the model tags
// candidates, the engine ranks them against the profile and topics).
//
...
//     directory resource base for relative references
import { readdir, readFile } from 'node:fs/promises'
import { fileURLToPath } from 'node:url'
import { dirname, join } from 'node:path'
import { createBookSearchTool } from '../src/book-search.js'
...
export function apply(ctx) {
  ...
  ctx.tools.register(createBookSearchTool())
  ctx.tools.register(createBookRankTool())
